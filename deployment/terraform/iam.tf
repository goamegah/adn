...
# 5. Allow Cloud Build SA to access GitHub PAT stored in Secret Manager
...
resource "google_secret_manager_secret_iam_member" "cloudbuild_secret_access" {
  ...
  member    = "serviceAccount:service-${data.google_project.cicd.number}@gcp-sa-cloudbuild.iam.gserviceaccount.com"
